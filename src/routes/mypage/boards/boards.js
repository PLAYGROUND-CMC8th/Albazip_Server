...
router.get('/',userUtil.LoggedIn, async (req,res)=> {
    ...
    if(req.job[1] == 'S') {
        ...
        await position.findOne({attributes: [ 'title' ], where: {id: req.job.substring(1)}})
            .then((positionData) => {
    ...
    let writerName;
    await user.findOne({attributes: [ 'last_name', 'first_name' ], where: {id: req.id}})
        .then((userData) => {
    ...
    if( postData ) {
        for (const pdata of postData) {
            ...
            let commentCount;
            await comment.count({where: {status: [1, 2], board_id: pdata.id}})
                .then((count) => {
